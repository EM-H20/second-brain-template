...
2. Open Claude Code and run it (once)
   ```
   /setup-vault
   ```
...
1. From that project's root, re-run the installer (answer `y`)
   ```bash
   npx github:EM-H20/second-brain-template
   ```
2. Open Claude Code or Codex and run it — every time you update (or just say "apply the template update")
   ```
   /update-vault
   ```
   Codex: `$update-vault`
...
- **Bounded retrieval:** filter YAML frontmatter, narrow with `rg`, then open only the final notes.
...
search and the graph to keep the vault light. Its originals are Git-ignored by default; opt in
only when the repository is private and tracking them is intentional.
...
1. 在该项目根目录重新运行安装程序（输入 `y`）
   ```bash
   npx github:EM-H20/second-brain-template
   ```
2. 打开 Claude Code 或 Codex 并运行 —— 每次更新后都要执行（或直接说"应用模板更新"）
   ```
   /update-vault
   ```
   Codex：`$update-vault`
...
- **限定检索范围：** 先过滤 YAML frontmatter，再用 `rg` 缩小范围，最后只打开命中的笔记。
  `_sources/` 中的原文不会参与检索。
...
以保持知识库的轻量。原文默认被 Git 忽略；只有在私有仓库且明确需要时才应纳入版本控制。
...
1. そのプロジェクトのルートでインストーラーを再実行（`y` を押す）
   ```bash
   npx github:EM-H20/second-brain-template
   ```
2. Claude Code か Codex を開いて実行 —— 更新のたびに（または「テンプレートの更新を反映して」）
   ```
   /update-vault
   ```
   Codex：`$update-vault`
...
- **検索範囲を限定：** YAML frontmatter で絞り、`rg` で候補を減らしてから、最終候補だけを
...
除外され、ボールトを軽量に保ちます。原文はデフォルトで Git の追跡対象外です。非公開リポジトリで
意図的に保存する場合だけ追跡してください。
...
1. 그 프로젝트 루트에서 설치기를 다시 실행 (`y`)
   ```bash
   npx github:EM-H20/second-brain-template
   ```
2. Claude Code·Codex를 열고 실행 — 업데이트할 때마다 (또는 "업데이트 반영해")
   ```
   /update-vault
   ```
   Codex: `$update-vault`
...
- **검색 범위 제한:** YAML frontmatter로 거르고 `rg`로 후보를 좁힌 뒤 최종 노트만 연다.
...
제외되어 볼트를 가볍게 유지한다. 원본은 기본적으로 Git 추적에서 제외하며, 비공개 저장소에서
의도적으로 보관할 때만 추적한다.
