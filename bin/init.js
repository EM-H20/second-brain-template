...
const AGENTS_HOOK_ID = '.agents/hooks/session-context.mjs';
...
function planGeminiMd() {
  const to = target('GEMINI.md');
  if (!fs.existsSync(to)) return { kind: 'gemini-create', rel: 'GEMINI.md', label: '신규' };
  if (fs.readFileSync(to, 'utf8').includes(IMPORT_LINE)) return { kind: 'keep', rel: 'GEMINI.md' };
  return { kind: 'gemini-append', rel: 'GEMINI.md', label: 'import 한 줄 추가' };
}

// .claude/settings.json 은 사용자 소유 파일이다 (permissions, 다른 플러그인의 훅, env).
// 통째로 덮지 않고 SessionStart 항목 하나만 멱등하게 병합한다 —
// planClaudeMd/planAgentsMd/planGeminiMd 가 CLAUDE.md·AGENTS.md·GEMINI.md 에 한 줄만 덧붙이는 것과 같은 방식.
...
// .agents/hooks.json (Antigravity/Gemini) 병합
function planAgentsHooks() {
  ...
  try {
    ...
    return { kind: 'agents-hooks-unparsable', rel };
  }
  if (!cur || typeof cur !== 'object' || Array.isArray(cur)) return { kind: 'agents-hooks-unparsable', rel };
  if (JSON.stringify(cur).includes(AGENTS_HOOK_ID)) return { kind: 'keep', rel };
  return { kind: 'agents-hooks-merge', rel, label: 'PreInvocation 훅 1개 추가 (.bak 백업)' };
}
...
function buildPlan() {
  ...
  plan.push(planAgentsHooks());
  plan.push(planClaudeMd());
  plan.push(planAgentsMd());
  plan.push(planGeminiMd());
...
function applyAction(a) {
  ...
  } else if (a.kind === 'claude-create' || a.kind === 'gemini-create') {
  ...
  } else if (a.kind === 'agents-hooks-merge') {
    const raw = fs.readFileSync(to);
    const cur = JSON.parse(raw.toString('utf8'));
    const src = JSON.parse(fs.readFileSync(path.join(SRC, '.agents/hooks.json'), 'utf8'));
    cur['second-brain-session-context'] = src['second-brain-session-context'];
    write(to + '.bak', raw);
    write(to, JSON.stringify(cur, null, 2) + '\n');
...
confirm((ok) => {
  ...
  const agentsHooks = plan.find((a) => a.rel === '.agents/hooks.json');
  if (agentsHooks && (agentsHooks.kind === 'copy' || agentsHooks.kind === 'agents-hooks-merge')) {
    console.log('✓ Antigravity (Gemini) 세션 컨텍스트 훅 등록됨 (.agents/hooks.json)\n');
    if (agentsHooks.kind === 'agents-hooks-merge') {
      console.log('  기존 훅 설정 백업: .agents/hooks.json.bak\n');
    }
  } else if (agentsHooks && agentsHooks.kind === 'agents-hooks-unparsable') {
    console.log('! .agents/hooks.json 을 파싱하지 못해 훅 등록을 건너뛰었습니다.');
    console.log('  훅 스크립트는 설치됐습니다. .agents/hooks.json 에 "second-brain-session-context" 를 추가하세요.\n');
  }
  ...
  console.log('  주제가 있으면 주입될 JSON이, 빈 볼트면 아무것도(또는 빈 배열이) 출력됩니다.');
});
