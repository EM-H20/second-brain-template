...
- Full (`/cluster`): **run the integrity check below** — its mechanical items are
  done by `node second-brain/tools/vault.mjs check` (read-only; report its output
  as-is instead of re-scanning every frontmatter by hand) — then rebuild every
  cluster note, merge topics that turned out to be duplicates (update `_topics.md`
  and retag affected notes).
